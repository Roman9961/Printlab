...
class Amo
{
        ...
        if(!$isAmo){
            ...
            if(isset($this->data['orderId'])){
                ...
                $lead->addCustomField(144157,
                    $this->getTemplate($this->data, 'order_id')
                );

                $lead->addCustomField(599763,
                    $this->getTemplate($this->data, 'delivery_id')
                );
    ...
    private function getTemplate($params, $template_name)
    {
        $trans =array(
            'white'=>'белая',
        ...
        $message = str_replace('%pay_status%',($params['pay_status']=='success'? 'Оплачен' : 'Ожидает оплаты'), $message);
        $message = str_replace('%pay_method%',$params['user']['payment_method'], $message);
        $message = str_replace('%comment%',(isset($params['delivery']['comment']) ? $params['delivery']['comment'] : '-'), $message);
        ...
        $message = str_replace('%delivery_method%',$trans[$params['delivery']['method']], $message);
        $message = str_replace('%delivery_name%',$params['delivery']['name'], $message);
        $message = str_replace('%delivery_phone%',$params['delivery']['phone'], $message);
        $message = str_replace('%delivery_city%',(isset($params['delivery']['city']) ? $params['delivery']['comment'] : 'Киев'), $message);
        $message = str_replace('%delivery_warehouse%',(isset($params['delivery']['warehouse']) ? $params['delivery']['warehouse'] : '-'), $message);
